...
#            f.write('@C,O\t{0}\n'.format(value[k]))
        ...
        for k,res in enumerate(residue):
            if np.size(chain)==np.size(residue):
                f.write('\t:{0}@N,H,HN,CA/pdbSegment={1}|'.format(res,chain[k][:4]))
            elif chain is not None:
                f.write('\t:{0}@N,H,HN,CA/pdbSegment={1}|'.format(res,chain[:4]))
            else:
...
#            f.write('@N,H,HN,CA|')
            ...
            if np.size(chain)==np.size(residue):
                f.write(':{0}@C,O/pdbSegment={1}'.format(res-1,chain[k][:4]))
            elif chain is not None:
                f.write(':{0}@C,O/pdbSegment={1}'.format(res-1,chain[:4]))
            else:
...
#        string=string+'|:{0}.{1}@N,H,HN,CA'.format(res0,chain)
        string=':{0}&@C,O/pdbSegment={1}'.format(res1,chain[:4])
        string=string+'|:{0}@N,H,HN,CA/pdbSegment={1}'.format(res0,chain[:4])
...
#                if chain0 is not None:
#                    WrCC(f,'sel {0}'.format(bond_sel_string(atoma,atomb,chain0)))
#                else:
...
#            WrCC(f,'2dlabels create maximum text scaled by {0:.3f} color black size 20 xpos .1 ypos .1'.format(scaling))
    ...
    os.spawnl(os.P_NOWAIT,chimera_path(),chimera_path(),full_path)
#    except: #Clean up the files with random numbers if crash occurs
#        if os.path.isfile(full_path):
#            os.remove(full_path)
#        else:
#            print('Chimera script was not generated')
#        path=get_path('attr_radius{0:06d}.txt'.format(rand_index))
#        if os.path.isfile(path):
#            os.remove(path)
#        else:
#            print('Attribute file was not generated')
#        path=get_path('attr_rho{0:06d}.txt'.format(rand_index))
#        if os.path.isfile(path):
#            os.remove(path)
#        
