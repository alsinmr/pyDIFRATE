...
import copy
import Sensitivities.functions.DynamicModels as dm
...
class Model(object):
    def __init__(self,tc=None,z=None):
        ...
        "Set the correlation time axis"
        if tc:
            if np.size(tc)==3:
                self.__tc=np.logspace(np.log10(tc[0]),np.log10(tc[1]),int(tc[2]))
            else:
                self.__tc=np.array(tc)
        elif z:
            self.__tc=np.logspace(z[0],z[1],int(z[2])) if np.size(z)==3 else np.array(z)
        else:
            self.__tc=np.logspace(-14,-3,200)
        
                
        
        "Initiate various data storage"        
        self.__Reff=list()
        self.__R0=list()
        self.__ReffCSA=list()
        self.__R0CSA=list()
    ...
    @property
    def z(self):
        "Returns the log-correlation time axis"
